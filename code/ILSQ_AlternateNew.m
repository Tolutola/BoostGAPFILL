function [Lambda,scores] = ILSQ_Alternate(wtrain,test,k,ith_experiment,hltest,num_prediction,max_iter,blacklist)
%  wtrain: the weighted train adjacency matrix
%  test: no use, for consistency with other link prediction algorithms (can be ignored)
%  k: the number of latent factors used in matrix factorization
%  max_iter: the maximum number of iterations allowed
%%
mask = spones(wtrain)==0;
[rr,cc] = size(hltest);


U1 = [];
for i=1:cc
    u = hltest(:,i);
    u = u*u';
    u = sparse(u(:));    
    U1 = [U1,u];
end


scores = zeros(cc,1);   %initialize scores
DS = zeros(1,max_iter);  %record the ||newscores - scores||
Scores = [];

for iter = 1:max_iter
    iter;
    
    %completion step
    
%     [Y,I] = sort(scores,1,'descend');
%     thres = scores(I(num_prediction*2));
%     scores1 = scores;
%     scores1(scores1<thres) = 0;
%     wtrain1 = wtrain + (hltest*diag(scores1)*hltest').*(~mask);
    
    wtrain1 = wtrain + (hltest*diag(scores)*hltest').*(~mask);
    [~,~,~,wdA] = evalc('FM(wtrain1,test,k,ith_experiment);');
    
    %matching step
%     if nargin == 8
%         [~,newscores] = ILSQ_Match(wdA,U1,num_prediction,rr,cc,mask,0,blacklist);    
%     else
%         [~,newscores] = ILSQ_Match(wdA,U1,num_prediction,rr,cc,mask,0);
%     end
    
    %matching with flux constraints
    [~,newscores] = ILSQ_Match_Flux(wdA,U1,num_prediction,rr,cc,mask,0,hltest,-1000*ones(cc,1),1000*ones(cc,1));
    
    %stop criterion
    ds = norm(newscores - scores);
    DS(iter) = ds;
    if ds>min(DS(1:max(iter-1,1)))        %if stops because of stop criterion, calculate the mean scores before this iteration
        break
    end
    scores = newscores;
    Scores = [Scores,scores];
end
scores = mean(Scores(:,1:max(end-1,1)),2);
%scores = Scores(:,end-1);

Lambda = zeros(cc,1);
[~,I] = sort(scores,1,'descend');
if num_prediction > 1
    Lambda(I(1:num_prediction)) = 1;    %only keep hl with top scores
    Lambda = logical(Lambda);
else
    Lambda(scores>num_prediction) = 1;
    Lambda = logical(Lambda);
end

DS
