function MatricesSUX = assignRxnWeights2(MatricesSUX,weights,WeightsPerRxn, NoWeight)
...
%         defWeight=max(WeigthsPerRxn.var);
%     else
...
if testmode
    %use default values in FASTGAPFILL
     weights.MetabolicRxns = .1; % Kegg metabolic reactions
    weights.ExchangeRxns = .5; % Exchange reactions
    weights.TransportRxns = 10; % Transport reactions
end
...
if exist('WeightsPerRxn','var') && isfield(WeightsPerRxn,'rxns')
    if ~isempty(WeightsPerRxn.rxns)
        ...
        for k = 1 : length(WeightsPerRxn.rxns)
            ...
            if ~isempty(R)
                MatricesSUX.weights(R) = WeightsPerRxn.var(k);
            end
        end
    end
end